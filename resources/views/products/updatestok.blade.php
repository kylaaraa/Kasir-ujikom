...
<div class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-40 z-50">
    <div class="bg-white w-full max-w-4xl p-8 rounded-xl shadow-lg relative">
        <h2 class="text-3xl font-semibold text-gray-800 mb-8 text-center">Edit Stok Produk</h2>

        <a href="{{ route('products.index') }}" class="absolute top-4 right-4 text-gray-500 hover:text-gray-800 text-2xl font-bold">
            &times;
        </a>

        <form action="{{ route('products.updatestok', $product->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-2 gap-8">
                ...
                <!-- Left Side (Nama Produk) -->
                <div class="w-full space-y-6">
                    <!-- Nama Produk -->
                    <div>
                        <label for="nama_produk" class="block text-sm font-bold text-gray-700 mb-2">Nama Produk</label>
                        <input type="text" id="nama_produk" name="nama_produk" value="{{ $product->nama_produk }}" disabled
